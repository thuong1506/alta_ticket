...
const TicketManagement: React.FC = () => {
  ...
  const exportFamilyExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(familyData); // familyData là mảng dữ liệu của bảng gói gia đình
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'GoiGiaDinh');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(data, 'GoiGiaDinh.xlsx');
  };
  
  const exportEventExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(evenData); // eventData là mảng dữ liệu của bảng gói sự kiện
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'GoiSuKien');
    const excelBuffer = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    const data = new Blob([excelBuffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    saveAs(data, 'GoiSuKien.xlsx');
  };
  ...
  const [viewMode, setViewMode] = useState<'family' | 'event'>('family');
  const [showFilterModal, setShowFilterModal] = useState(false);
  const [showEventTable, setShowEventTable] = useState(false);
  const [showFamilyTable, setShowFamilyTable] = useState(true); // Ban đầu hiển thị bảng gói gia đình
  ...
  const handleAnchorClick = (item: string) => {
    if (item === 'Gói sự kiện') {
      setViewMode('event'); // Cập nhật giá trị của viewMode khi chuyển sang chế độ xem sự kiện
      setShowEventTable(true);
      setShowFamilyTable(false);
    } else if (item === 'Gói gia đình') {
      setViewMode('family'); // Cập nhật giá trị của viewMode khi chuyển sang chế độ xem gia đình
      ...
      <Row>
        ...
        <Col span={19}>
          ...
          <div style={ticketListContainerStyle}>
            ...
            {showEventTable && (
              <Table columns={columns} dataSource={evenData} pagination={{ pageSize: 6 }} />
            )}
            {showFamilyTable && (
              <Table columns={familyColumns} dataSource={familyData} pagination={{ pageSize: 6 }} />
            )}
